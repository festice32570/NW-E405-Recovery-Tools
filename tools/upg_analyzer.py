...
def parse_bytes(data: bytes) -> UpgInfo:
    ...
    if size >= 0x78:
        for off in range(0x50,0x78,8):
            # Record format observed in official E40X/E50X packages:
            # first byte is record number, final dword is BE length.
            lengths.append(int.from_bytes(data[off+4:off+8], 'big'))
    lengths=tuple(lengths)
    structure_ok=(magic_ok and sony_ok and lengths == EXPECTED_SECTION_LENGTHS and sum(lengths)==size)
    ...
    return UpgInfo(size,sha,magic_ok,sony_ok,model,lengths,structure_ok,exact,reason)
