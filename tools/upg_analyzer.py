...
def parse_bytes(data: bytes) -> UpgInfo:
    ...
    if size >= 0x78:
        for off in range(0x50,0x78,8):
            ...
            # type is LE32, length is BE32. Regional/revision variants can
            # legitimately vary record lengths while preserving semantic types.
    ...
    structure_ok=(magic_ok and sony_ok and types == EXPECTED_SECTION_TYPES and sum(lengths)==size)
    japanese_layout_ok=(structure_ok and lengths == EXPECTED_SECTION_LENGTHS)
    exact=(japanese_layout_ok and model==E40X_MODEL_ID and size==E40X_J_SIZE and sha==E40X_J_SHA256)
    ...
    elif sum(lengths) != size: reason='Descriptor lengths do not match file size'
    elif lengths != EXPECTED_SECTION_LENGTHS:
        reason='Structurally valid E40X package with alternate section lengths; not approved Japanese v2.0 image'
    else:
        reason='Structurally E40X-like with Japanese layout but hash is not approved Japanese v2.0 image'
    return UpgInfo(size,sha,magic_ok,sony_ok,model,types,lengths,structure_ok,japanese_layout_ok,exact,reason)
...
def main(argv):
    ...
    print('japanese_layout_ok:',info.japanese_layout_ok)
