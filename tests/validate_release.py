...
allc=s+'\n'+fwp
checks={
 'v0.8.3 recovery build':'NW-E405 Recovery Tool v0.8.3-dev' in s,
 ...
 'SONYICD exact transfer/status gate':'rd.returnedDataLen!=0x74 || rd.dataLen!=0x74' in s and 'rd.data[0x0F]' in s,
 ...
 'FC05 authentic exact GetDeviceId probe':'static const BYTE cdb[12]={0xFC,0x00,0x05,0x72,0x6F,0x67,0x61,0x00,0x00,0x10,0x00,0x00};' in s and 'SendCdb(h,cdb,12,16)' in s and 'rd.returnedDataLen!=16 || rd.dataLen!=16' in s,
 'FC05 signature-less ordinary diagnostic removed':'cdb[2] = 0x05' not in s and 'cdb[2]=0x05' not in s,
 'FC05 private/public boundary':'SavePrivateBlob(L"FC05_DEVICEID",rd.data,16,g_fc05DeviceIdSha256)' in s and 'E40X FC/05+roga response SHA-256 only' in s and 'ShowScsiResultRedacted' in s,
 ...
 'SPTI actual transfer length used':'r.returnedDataLen = (ok && dataLen) ? pkt.sptd.DataTransferLength : 0;' in s and 'DWORD actualLen = r.returnedDataLen;' in s and 'r.dataLen = actualLen;' in s,
